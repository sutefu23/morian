...
import { Table, Thead, Tbody, Tr, Th, Td, Text, Input, Container, Button, Flex, InputRightElement, InputGroup } from '@chakra-ui/react'
...
const UserManage = () => {
  ...
  useAspidaQuery(apiClient.user, {
    onSuccess: async (users) => {
      const maxId = users.reduce((prevId, curr) => (prevId < curr.id ? curr.id : prevId), 0)
  ...
  return (
    <>
      <Container mt="50px" border="solid 1px #eee" w="60vw" maxW="60vw">
        <Table variant="striped" colorScheme="gray" w="100%">
          ...
          <Tbody>
            {users.map((user) => (
              <Tr key={user.id}>
                <Td>
                  <Text ml="10px">{('0000' + user.id).slice(-4)}</Text>
                </Td>
                <Td>{user.name}</Td>
                <Td>
                  <Flex>
            ...
            <Tr>
              <Td>
                <Input bgColor="white" type="text" readOnly defaultValue={newUser?.id && ('0000' + newUser.id).slice(-4)}></Input>
              </Td>
              <Td>
                <Input
                  bgColor="white"
