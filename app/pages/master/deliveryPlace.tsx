...
const DeliveryPlaceTypeManage = () => {
  ...
  useAspidaQuery(apiClient.master.deliveryPlace, {
    onSuccess: async (masters) => {
  ...
  const handleModifyAddress = useCallback(
    async (id: number, newAddhandleModifyAddress: string) => {
  ...
  return (
    <>
      <Container mt="50px" border="solid 1px #eee" w="80vw" maxW="80vw">
        <Text fontSize="12px" textAlign="right" color="gray.500" mt="10px">
          ※並び順と住所の変更は即時保存されます
        </Text>
        <Table variant="striped" colorScheme="gray" w="100%">
          ...
          <Tbody>
            ...
            <Tr>
              <Td>
                <Input bgColor="white" type="text" readOnly defaultValue={newDeliveryPlaceType?.id}></Input>
              </Td>
              <Td>
                <Input
                  bgColor="white"
