...
const SupplierManage = () => {
  ...
  useAspidaQuery(apiClient.supplier,{onSuccess:async (suppliers) => {
    const maxId = suppliers.reduce((prevId, curr)=>(
  ...
  const setSupplier = useCallback((index:number, key:keyof SupplierDTO, value: SupplierDTO[keyof SupplierDTO]) => {
  ...
  const submitModifySupplier = useCallback(async (index: number) => {
