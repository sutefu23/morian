...
const MasterManage = () => {
  ...
  useEffect(() =>{
    ...
    (async () => {
      if (masterTypes.findIndex(m => m.key === masterType as unknown ) < 0){
        throw Error("URLパラメーターがマスタ形式ではありません"+masterType)
      }
      ...
      const data = await apiClient.master[type].get()
      const body = data.body
      setMasters(body)

      const title = masterTypes.find(m => m.key === type)?.name
