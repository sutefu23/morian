...
import { Button, Table, Thead, Tbody, Tr, Th, Td, Heading, HStack, Box, InputGroup, useDisclosure, InputLeftAddon, Input } from '@chakra-ui/react'
    ...
    <>
      <div>
        ...
        <HStack mt={10} mb={10}>
          <Box>
            <InputGroup>
              <InputLeftAddon bgColor="blue.100" ml="10" aria-required>
                発注日検索
              </InputLeftAddon>
              <Input
          ...
          <Box>
            <InputGroup>
              <InputLeftAddon bgColor="blue.100" ml="10">
                仕入先検索
              </InputLeftAddon>
              <SupplierSelect
                onSelect={(selected) => {
          ...
          <Box>
            <Button ml="10" onClick={handleSearch}>
              検索
            </Button>
          </Box>
        </HStack>
                          ...
                          <Td textAlign="right">
                            <Button
                              ...
                              onClick={(e) => {
                                ...
                                const issueData = { ...issue, id: undefined, managedId: undefined, date: new Date(), userId: user?.id, userName: user?.name, innerNote: '', issueItems: items }
                                router.push({
        ...
        <aside>
          <RightDrawer isOpen={isRightOpen} onClose={onRightClose} width="30vw">
            {issues && <IssueDetail item={issues[issueIndex]?.issueItems[itemIndex]} issue={issues[issueIndex]} editable={false} onSuccessDelete={refetch}></IssueDetail>}
          </RightDrawer>
        </aside>
      </div>
    </>
