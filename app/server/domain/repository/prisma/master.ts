...
export class GradeRepository implements IGradeRepository {
  async create(entity: GradeType): Promise<GradeType | FieldNotFoundError> {
    const result = await prisma.grade.create({ data: entity })
  ...
  async update(
    id: number,
    entity: Partial<GradeType>
  ): Promise<GradeType | FieldNotFoundError> {
    const result = await prisma.grade.update({ where: { id }, data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
...
export class UnitRepository implements IUnitRepository {
  async update(
    id: number,
    entity: Partial<UnitType>
  ): Promise<UnitType | Error> {
    const result = await prisma.unit.update({ where: { id }, data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
  ...
  async create(entity: UnitType): Promise<UnitType | FieldNotFoundError> {
    const result = await prisma.unit.create({ data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
...
export class WarehouseRepository implements IWarehouseRepository {
  async update(
    id: number,
    entity: Partial<WarehouseType>
  ): Promise<WarehouseType | Error> {
  ...
  ): Promise<WarehouseType | FieldNotFoundError> {
    const result = await prisma.warehouse.create({ data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
...
export class DeliveryPlaceRepository implements IDeliveryPlaceRepository {
  async update(
    id: number,
    entity: Partial<DeliveryPlaceType>
  ): Promise<DeliveryPlaceType | Error> {
  ...
  async create(
    entity: DeliveryPlaceType
  ): Promise<DeliveryPlaceType | FieldNotFoundError> {
    const result = await prisma.deliveryPlace.create({ data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
...
export class ReasonRepository implements IReasonRepository {
  async update(
    id: number,
    entity: Partial<ReasonType>
  ): Promise<FieldNotFoundError | ReasonType> {
    const result = await prisma.reason.update({ where: { id }, data: entity })
  ...
  async create(entity: ReasonType): Promise<ReasonType | FieldNotFoundError> {
    const result = await prisma.reason.create({ data: entity })
    if (!result?.id || !result?.name || !result?.order || !result.status) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
  ...
  async findAll(): Promise<ReasonType[] | FieldNotFoundError> {
    ...
    const reasons = result.map((r) => {
      return {
        id: r.id,
        name: r.name as 入庫理由 | 出庫理由,
        status: r.status,
        order: r.order
      }
    })
...
export class WoodSpeciesRepository implements ISpeciesRepository {
  async update(
    id: number,
    entity: Partial<WoodSpeciesType>
  ): Promise<WoodSpeciesType | Error> {
    const result = await prisma.species.update({ where: { id }, data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
  ...
  async create(
    entity: WoodSpeciesType
  ): Promise<WoodSpeciesType | FieldNotFoundError> {
    const result = await prisma.species.create({ data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
...
export class ItemTypeRepository implements IItemTypeRepository {
  async update(
    id: number,
    entity: Partial<ItemTypeType>
  ): Promise<ItemTypeType | Error> {
    const result = await prisma.itemType.update({ where: { id }, data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
  ...
  async create(
    entity: ItemTypeType
  ): Promise<ItemTypeType | FieldNotFoundError> {
    const result = await prisma.itemType.create({ data: entity })
    if (!result?.id || !result?.name || !result?.order) {
      return new FieldNotFoundError('データが見つかりません')
    }
    const data = {
