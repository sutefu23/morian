...
const EditHistory = ({
  ...
  useEffect(() => {
    setDefault()
  }, [defaultData, itemId, setDefault, setHistoryData, user?.id, user?.name])
    ...
    >
      ...
      <VStack align="left" pl="10">
        <HStack>
          <Box>
            <InputGroup>
              <InputLeftAddon>入出庫日</InputLeftAddon>
              <Input
                required
                type="date"
                value={
                  historyData.date
                    ? dayjs(historyData.date).format('YYYY-MM-DD')
                    : undefined
                }
                onChange={(e) => {
        ...
        <HStack>
          <Box>
            <InputGroup>
              <InputLeftAddon>ステータス</InputLeftAddon>
              <StatusSelect
                required
        ...
        <HStack>
          <Box>
            <InputGroup>
              <InputLeftAddon>理由</InputLeftAddon>
              <ReasonSelect
                required
                ...
                filter={
                  onlyUseSettledReason
                    ? (r) =>
                        r.name !== 出庫理由.見積 && r.name !== 出庫理由.受注予約
                    : undefined
                }
                onSelect={(e) => {
                  const reasonBookId = StockReason.find(
                    (r) => r.name === 出庫理由.受注予約
                  )?.id
                  const reasonEstimateId = StockReason.find(
                    (r) => r.name === 出庫理由.見積
                  )?.id
                  setIsBook(
                    Number(e.currentTarget.value) === reasonBookId ||
                      Number(e.currentTarget.value) === reasonEstimateId
                  )
                  updateField<'reasonId'>(
                    'reasonId',
              ...
              <Box width="75vw">
                <InputGroup>
                  ...
                  <Input
                    type="date"
                    value={
                      historyData.bookDate
                        ? dayjs(historyData.bookDate).format('YYYY-MM-DD')
                        : undefined
                    }
                    onChange={(e) => {
                      updateField<'bookDate'>(
                        'bookDate',
                ...
                <InputGroup>
                  ...
                  <UserSelect
                    ...
                    onSelect={(e) => {
                      ...
                      setHistoryData({
                        ...historyData,
                        bookUserId: Number(e.currentTarget.value),
                        bookUserName: options[selectedIndex].innerHTML
                      })
                    }}
                  />
                </InputGroup>
